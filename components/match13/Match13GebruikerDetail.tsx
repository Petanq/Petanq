...
export function Match13GebruikerDetail({ gebruiker }: { gebruiker: Match13GebruikerMetToernooien }) {
  ...
  async function opslaan(e: React.FormEvent) {
    ...
    if (!naam.trim()) return;
    setBezig(true);
    await match13NaamWijzigen(gebruiker.id, naam.trim());
    setBezig(false);
    setBewerken(false);
    router.refresh();
  }
    ...
    <div className="match13-lijst-pagina">
      <div className="match13-lijst-head" style={{ display: "block" }}>
        <nav className="match13-broodkruimel">
          ...
          <span>/</span>
          <span className="huidig">{gebruiker.naam}</span>
        </nav>

        {bewerken ? (
          <form className="name-edit" style={{ margin: "0.3rem 0" }} onSubmit={opslaan}>
            <input autoFocus value={naam} onChange={(e) => setNaam(e.target.value)} />
            <button type="submit" className="link-btn" disabled={bezig}>
              {t.match13.opslaan}
            </button>
            <button type="button" className="link-btn" onClick={() => { setBewerken(false); setNaam(gebruiker.naam); }}>
              {t.beheer.annuleren}
            </button>
          </form>
        ) : (
        ...
        <p style={{ color: "var(--ink-muted)", margin: 0 }}>{gebruiker.email}</p>
      </div>
