...
export function TournamentCard({ toernooi }: { toernooi: Toernooi }) {
  ...
  return (
    <Link
      ...
      className="group relative flex flex-col gap-3 overflow-hidden rounded-2xl border-[1.5px] border-rand bg-white p-4 transition-all hover:-translate-y-0.5 hover:border-blauw-3/40 hover:shadow-[0_8px_24px_rgba(26,68,128,0.1)] sm:grid sm:grid-cols-[56px_1fr_auto] sm:items-center sm:gap-4"
    >
      <span
      ...
      <div className="flex items-center gap-3 sm:contents">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-0.5 flex items-center gap-1.5 text-[0.72rem] font-bold uppercase tracking-wide text-blauw-2">
            <span className="min-w-0 truncate">{toernooi.clubnaam}</span>
            {toernooi.open_toernooi && (
              <span className="shrink-0 whitespace-nowrap rounded-full bg-[#f0fdfa] px-2 py-0.5 text-[0.62rem] font-bold uppercase tracking-wide text-[#0d9488]">
                {t.lijst.openBadge}
              </span>
            )}
          </div>
