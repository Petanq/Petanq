...
export function googleAgendaLink(toernooi: Toernooi, naam: string, datumOverride?: string, uurOverride?: string): string {
  const { start, eind } = tijdstippen(datumOverride ?? toernooi.datum, uurOverride ?? toernooi.uur, STANDAARD_DUUR_UUR);
