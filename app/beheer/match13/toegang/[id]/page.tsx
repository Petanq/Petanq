...
import { haalMatch13GebruikerMetToernooien } from "@/actions/match13-toegang";
...
export default async function Match13GebruikerPagina({ params }: { params: Promise<{ id: string }> }) {
  ...
  const gebruiker = await haalMatch13GebruikerMetToernooien(id);
  if (!gebruiker) redirect("/beheer/match13/toegang");

  return <Match13GebruikerDetail gebruiker={gebruiker} />;
}
