-- Qpetanque - alles-in-een migratie. Veilig om meermaals uit te voeren: elke
-- stap controleert eerst of hij al gebeurd is. Combineert migraties 0015 t/m 0019.

-- 1. Categorie "recreanten" toevoegen (indien nog niet gebeurd)
do $$
begin
  if not exists (select 1 from pg_enum where enumlabel = 'recreanten') then
    alter type categorie_enum add value 'recreanten';
  end if;
end $$;

-- 2. Kolom "open_toernooi" op toernooien (indien nog niet gebeurd)
alter table toernooien add column if not exists open_toernooi boolean not null default false;

-- 3. Kolommen "telefoon" en "openingsuren" op clubs (indien nog niet gebeurd)
alter table clubs add column if not exists telefoon text;
alter table clubs add column if not exists openingsuren text;

-- ===================== Clubgegevens (voorheen 0018) =====================
...
-- ===================== PC Terheiden correctie (voorheen 0019) =====================
